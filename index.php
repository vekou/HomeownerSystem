        ...
        case "charges":
            if((!is_null(filter_input(INPUT_GET, "id")))&&(isLoggedIn()) && (checkPermission(DT_PERM_CHARGE_VIEW)||  checkPermission(DT_PERM_PAYMENT_ADD)))
            {
                ...
                if(!$lid){
                    ...
                    $stmt=$conn->prepare("SELECT a.id, a.dateposted, a.description, a.amount, SUM(COALESCE(c.amountpaid,0)) AS amtpaid FROM charges a LEFT JOIN ledgeritem c ON a.id=c.chargeid WHERE a.lot=? GROUP BY a.id HAVING a.amount>SUM(COALESCE(c.amountpaid,0)) ORDER BY a.dateposted, a.description");
                }
                if($stmt === false) {
                    trigger_error('<strong>Error:</strong> '.$conn->error, E_USER_ERROR);
                }
                            ...
                            <div class="ui-corner-all">
                                ...
                                <div class="ui-body-a ui-content">
                                    <?php if($homeowner>0):?>
                                        ...
                                        <fieldset data-role="controlgroup" data-type="horizontal">
                                            <?php if(checkPermission(DT_PERM_PAYMENT_ADD)||checkPermission(DT_PERM_CHARGE_VIEW)): ?><a href="./charges?id=<?php echo $homeowner; ?>&lid=<?php echo $lid; ?>" data-role="button" data-icon="plus" data-inline="true" id="addPaymentBtns" data-theme="d">Add Payment</a><?php endif; ?>
            ...
            if(isLoggedIn() && checkPermission(DT_PERM_REPORTS_VIEW)){
                ...
                <div data-role="collapsibleset" data-theme="d" data-content-theme="a" id='reportpanel' class='ui-corner-all'>
                    <div data-role="collapsible">
                        <h3>Homeowners</h3>
                        <?php if(checkPermission(DT_PERM_HOMEOWNER_VIEW)): ?>
                        ...
                        <?php if(checkPermission(DT_PERM_HOMEOWNER_VIEW) && checkPermission(DT_PERM_PAYMENT_DELETE)): ?>
                        ...
                        <?php if(checkPermission(DT_PERM_HOMEOWNER_VIEW) ||  checkPermission(DT_PERM_PAYMENT_ADD) ||  checkPermission(DT_PERM_PAYMENT_VIEW)): ?>
                    ...
                    <div data-role="collapsible">
                        <h3>Lots</h3>
                        <?php if(checkPermission(DT_PERM_LOT_VIEW)): ?>
                        ...
                        <?php if(checkPermission(DT_PERM_LOT_VIEW) &&  checkPermission(DT_PERM_LOT_DELETE)): ?>
                        ...
                        <?php if(checkPermission(DT_PERM_LOT_VIEW) ||  checkPermission(DT_PERM_PAYMENT_VIEW) ||  checkPermission(DT_PERM_PAYMENT_ADD)): ?>
                    ...
                    <div data-role="collapsible">
                        ...
                        <?php if(checkPermission(DT_PERM_CHARGE_VIEW)||  checkPermission(DT_PERM_PAYMENT_VIEW)): ?>
                        ...
                        <?php if( checkPermission(DT_PERM_CHARGE_DELETE) && (checkPermission(DT_PERM_CHARGE_VIEW) || checkPermission(DT_PERM_PAYMENT_VIEW)) ): ?>
                        ...
                        <?php if(checkPermission(DT_PERM_CASHFLOW_VIEW)): ?>
                    ...
                    <div data-role="collapsible">
                        <h3>Users</h3>
                        <?php if(checkPermission(DT_PERM_USER_VIEW)): ?>
...
//                            return;
        ...
        //                $stmt->bind_param('ss',$postusername,$postpassword);
                    ...
                    case "lotlist":
                        $title="List of Lots";
                        $msg="";
                        $stmt=$conn->prepare('SELECT a.`code`, CONCAT(a.`housenumber`," ",a.`street`,", Lot ",a.`lot`," Block ",a.`block`," Phase ",a.`phase`) AS address, DATE_FORMAT(a.`dateacquired`,"%Y-%m-%d"),a.`lotsize`,COUNT(c.`id`) AS numberinhousehold, CONCAT(b.`lastname`,", ",b.`firstname`," ",SUBSTR(b.`middlename`,1,1),".") AS fullname FROM lot a LEFT JOIN homeowner b ON a.homeowner=b.id LEFT JOIN resident c ON c.household=a.id WHERE a.active=1 GROUP BY a.id');
                        if($stmt === false) {
                            trigger_error('<strong>Error:</strong> '.$conn->error, E_USER_ERROR);
                        }
                    ...
                    case "inactivelotlist":
                        ...
                        $msg="";
                        $stmt=$conn->prepare('SELECT a.`code`, CONCAT(a.`housenumber`," ",a.`street`,", Lot ",a.`lot`," Block ",a.`block`," Phase ",a.`phase`) AS address, DATE_FORMAT(a.`dateacquired`,"%Y-%m-%d"),a.`lotsize`,COUNT(c.`id`), CONCAT(b.`lastname`,", ",b.`firstname`," ",SUBSTR(b.`middlename`,1,1),".") AS fullname FROM lot a LEFT JOIN homeowner b ON a.homeowner=b.id LEFT JOIN resident c ON c.household=a.id WHERE a.active=0 GROUP BY a.id');
                        if($stmt === false) {
                            trigger_error('<strong>Error:</strong> '.$conn->error, E_USER_ERROR);
                        }
                        $resultcolumns = ["Lot Code","Address","Date Acquired","Lot Size","Number in Household","Owner"];
                    ...
                    case "homeownerbalances":
                        ...
                        $msg="";
                        $stmt=$conn->prepare('SELECT formatName(a.lastname,a.firstname,a.middlename), (SELECT COUNT(id) FROM lot WHERE homeowner=a.id) AS lots, coalesce((SELECT SUM(amount) FROM charges WHERE homeowner=a.id),0)-coalesce((SELECT SUM(x.amountpaid) FROM ledgeritem x LEFT JOIN ledger y ON x.ledgerid=y.id WHERE y.homeowner=a.id AND y.active=1),0) AS balance FROM homeowner a WHERE a.active=1 GROUP BY a.id');
                        if($stmt === false) {
                            trigger_error('<strong>Error:</strong> '.$conn->error, E_USER_ERROR);
                        }
                    ...
                    case "lotbalances":
                        $title="Lot Arrears";
                        $msg="";
                        $stmt=$conn->prepare('SELECT a.code AS code, formatAddress(a.housenumber,a.lot,a.block,a.street,a.phase) AS address, a.lotsize AS lotsize, (a.lotsize*f.price) AS dues, formatName(b.lastname,b.firstname,b.middlename) AS homeowner, coalesce((SELECT SUM(amount) FROM charges WHERE lot=a.id AND active=1),0)-coalesce((SELECT SUM(x.amountpaid) FROM ledgeritem x INNER JOIN ledger y ON x.ledgerid=y.id WHERE y.homeowner=b.id),0) AS balance
			FROM lot a 
LEFT JOIN homeowner b ON a.homeowner=b.id 
                    ...
                    if(is_null($applieddues) || $applieddues>0)
                    {
                        ...
                        if($stmt3->num_rows>0)
                        {
                            ...
                            while($stmt3->fetch()){
                                ...
                                array_push($sqlwhere, "($lid,$lhomeowner,'Charges for $monthFormat',$ldue,0,1)");
                            }
...
//            header("Location: ./");
        ...
        default :
            ...
            if(!isLoggedIn())
            {
                ?>
                <div class="pagelogo">
                    <img src="images/staisabellogo.png" alt="Santa Isabel Logo"/>
                </div>
                ...
                <ul> 
                    ...
                    <li>Add monthly dues to homeowner's ledger including interest.</li>
                    <li>Monitor the Cash Flow of the subdivision.</li>
                    <li>Fine-grained user access permissions.</li>
                    <li>Encode the resident names associated with a household.</li>
                    <li>Generate a Billing Statement for each lot.</li>
                </ul>
                    </div>
                <?php
            }
